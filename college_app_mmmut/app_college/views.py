...
def home(request):
    ...
    if request.method=="POST":
        ...
        f.save()
        return redirect('/')
    return render(request, 'home.html',{'mydata':mydata,'notice':notice})

def sem(request):
    event = Event.objects.all()
    return render(request, 'sem.html',{'event':event})
...
def event_show(request,event_id):
    ...
    return render(request, 'event_show.html',{'data':data})

def workshop(request):
    web_data = Web_work.objects.all()
    cir_data = Circuitary_work.objects.all()
    return render(request, 'workshop.html',{'web_data':web_data,'cir_data':cir_data})

def gallery(request):
    gallery_images = Gallery_img.objects.all()
    return render(request, 'gallery.html',{'data':gallery_images})

def team(request):
    team_data = Team_info.objects.all()
    return render(request, 'team.html',{'data':team_data})
